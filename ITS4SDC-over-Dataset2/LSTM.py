...
with open('dataset_3559_road_characteristics.json', 'r') as f:
    ...
    class CustomEarlyStopping(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs=None):
            ...
            if val_accuracy is not None and val_accuracy >= 0.88:
                print(f"\nFold {fold_num} reached 87% validation accuracy. Stopping early.")
                self.model.stop_training = True

    # Train the model
    model.fit(X_train, y_train, batch_size=1024, epochs=400, validation_data=(X_test, y_test), verbose=1)
